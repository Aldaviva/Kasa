...
public class KasaOutletScheduleTest: AbstractKasaOutletTest {
    ...
    public async Task GetAll() {
        ...
        actual[0].StartTimeBasis.Should().Be(Schedule.Basis.StartOfDay);
        actual[0].TimeSinceBasis.Should().Be(TimeSpan.FromHours(3).Add(TimeSpan.FromMinutes(15)));
        ...
        actual[0].Repeat.Should().BeTrue();
        actual[0].WillSetOutletOn.Should().BeTrue();

        actual[1].Id.Should().Be("901F46B4FFC33957D2A5383F2F83BDE0");
        actual[1].StartTimeBasis.Should().Be(Schedule.Basis.Sunrise);
        actual[1].TimeSinceBasis.Should().Be(TimeSpan.FromMinutes(5));
        ...
        actual[1].Repeat.Should().BeTrue();
        actual[1].WillSetOutletOn.Should().BeFalse();

        actual[2].Id.Should().Be("13B98EDB80160333AF59C23B73036378");
        actual[2].StartTimeBasis.Should().Be(Schedule.Basis.Sunset);
        actual[2].TimeSinceBasis.Should().Be(TimeSpan.FromMinutes(10).Negate());
    ...
    public async Task Insert() {
        ...
        actual.StartTimeBasis.Should().Be(Schedule.Basis.StartOfDay);
        actual.TimeSinceBasis.Should().Be(TimeSpan.FromHours(12 + 7).Add(TimeSpan.FromMinutes(45)));
    ...
    public void SunRelative() {
        ...
        schedule.Repeat.Should().BeTrue();
        schedule.StartTimeBasis.Should().Be(Schedule.Basis.Sunset);
        schedule.TimeSinceBasis.Should().Be(TimeSpan.FromMinutes(5));
    ...
    public void NonRepeating() {
        ...
        foreach (Schedule schedule in schedules) {
            schedule.Repeat.Should().BeFalse();
            schedule.StartTimeBasis.Should().Be(Schedule.Basis.StartOfDay);
            schedule.TimeSinceBasis.Should().Be(TimeSpan.FromHours(3).Add(TimeSpan.FromMinutes(52)));
            schedule.WillSetOutletOn.Should().BeTrue();
